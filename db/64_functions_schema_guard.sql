-- Canonical-schema guard (#77 follow-through): the ag_catalog shadow bug ran
-- silently for months — stale Hexis function copies in ag_catalog outranked
-- their migrated public twins for every worker connection. Migration 0043
-- evicted the strays; this file makes recurrence impossible to miss:
--   1. schema_shadow_report()  — what is shadowed right now (doctor/CI/tests)
--   2. heal_schema_shadows()   — evict strays, loudly (migration-runner
--      postcondition after every apply)
--   3. an event trigger that REJECTS creating an ag_catalog function whose
--      name exists in public — the mistake fails at creation time, with the
--      cause and the fix in the error, instead of festering as a fossil.
