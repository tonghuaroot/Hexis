-- 0115: Source-document desk loading and DB-native desk search.
SET search_path = public, ag_catalog, "$user";

INSERT INTO config_defaults (key, value, description) VALUES
    ('memory.source_document_desk_chunk_chars', '8000'::jsonb, 'Default chunk size when loading source documents onto the RecMem desk')
...
DECLARE
    fn_sql TEXT;
BEGIN
    SELECT pg_get_functiondef('execute_memory_tool(text,jsonb)'::regprocedure)
    INTO fn_sql;

    IF fn_sql IS NULL THEN
        RAISE EXCEPTION 'execute_memory_tool(text,jsonb) is missing';
    END IF;

    fn_sql := replace(
        fn_sql,
        'WHERE v NOT IN (''turn'', ''memory'')) THEN',
        'WHERE v NOT IN (''turn'', ''memory'', ''desk'')) THEN'
    );
    fn_sql := replace(
        fn_sql,
    ...
    fn_sql := replace(
        fn_sql,
        'WHERE v NOT IN (''turn'', ''memory'')),',
        'WHERE v NOT IN (''turn'', ''memory'', ''desk'')),'
    );
...
SELECT upsert_prompt_module(
    'conversation',
    $pm0115$# Conversation System Prompt
...
$pm0115$,
    'Updated by 0115 source-document desk search',
    'services/prompts/conversation.md'
);

SELECT upsert_prompt_module(
    'heartbeat_agentic',
    $pm0115$# Heartbeat System Prompt (Agentic / Tool-Use)
...
SELECT upsert_prompt_module(
    ...
    $pm0115$# RLM Heartbeat Decision System Prompt
...
SELECT upsert_prompt_module(
    ...
    $pm0115$# RLM Slow Ingest System Prompt
