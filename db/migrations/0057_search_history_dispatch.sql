-- search_history joins the DB-native memory tools: execute_memory_tool
-- now owns its validation (time window, sources, browse detection), the
-- browse-vs-keyword limit policy, result shaping, and the loud-truncation
-- paging hint. The Python handler is dispatch-only.
