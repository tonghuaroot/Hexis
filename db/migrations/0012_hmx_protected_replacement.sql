...
-- Baseline mirror: db/53_hmx_protected_replacement.sql
...
BEGIN
    IF requested_id IS NULL THEN RAISE EXCEPTION 'audit record requires audit_id'; END IF;
    IF requested_type NOT IN ('protected_section_replacement','protected_section_verified','protected_section_reverted') THEN
    ...
    END IF;
    BEGIN requested_time := (p_record->>'event_time')::timestamptz;
    EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'audit record requires a valid event_time'; END;
    INSERT INTO protected_replacement_audit (
        audit_id,event_type,event_time,record,record_digest_v1,is_foreign_diagnostic,imported_at
    ) VALUES (
        requested_id,requested_type,requested_time,p_record,p_record_digest_v1,
        COALESCE(p_is_foreign_diagnostic,FALSE),p_imported_at
    ) ON CONFLICT (audit_id) DO NOTHING;
    GET DIAGNOSTICS inserted_count = ROW_COUNT;
    IF inserted_count = 1 THEN
        RETURN jsonb_build_object('status','inserted','audit_id',requested_id);
    END IF;
    SELECT record_digest_v1 INTO existing_digest FROM protected_replacement_audit WHERE audit_id=requested_id;
    IF existing_digest = p_record_digest_v1 THEN
        RETURN jsonb_build_object('status','duplicate','audit_id',requested_id);
    END IF;
    RETURN jsonb_build_object(
        'status','conflict','code','audit_integrity_conflict','audit_id',requested_id,
        'existing_digest_v1',existing_digest,'imported_digest_v1',p_record_digest_v1
    );
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION hmx_export_audit_records() RETURNS JSONB AS $$
    SELECT jsonb_build_object(
        'protected_replacement_audit',COALESCE((SELECT jsonb_agg(record ORDER BY event_time,audit_id) FROM protected_replacement_audit WHERE event_type='protected_section_replacement' AND NOT is_foreign_diagnostic),'[]'::jsonb),
        'protected_section_verified_audit',COALESCE((SELECT jsonb_agg(record ORDER BY event_time,audit_id) FROM protected_replacement_audit WHERE event_type='protected_section_verified' AND NOT is_foreign_diagnostic),'[]'::jsonb),
        'protected_replacement_reversion_audit',COALESCE((SELECT jsonb_agg(record ORDER BY event_time,audit_id) FROM protected_replacement_audit WHERE event_type='protected_section_reverted' AND NOT is_foreign_diagnostic),'[]'::jsonb),
...
DECLARE
    current_heartbeat BIGINT := COALESCE((SELECT heartbeat_count FROM heartbeat_state WHERE id=1),0);
    affected INTEGER;
BEGIN
    UPDATE hmx_pending_replacements SET
        status='timed_out',
        acknowledgement=jsonb_build_object('decision','timed_out','reason','both the wall-clock and heartbeat acknowledgement limits elapsed'),
        acknowledgement_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
    WHERE status IN ('pending','deferred') AND timeout_at<=CURRENT_TIMESTAMP
...
BEGIN
    ...
    SELECT jsonb_build_object(
        ...
        'records',COALESCE(jsonb_agg(jsonb_build_object(
            'replacement_id',replacement_id,'section',section,'source',source,
            'replacement_scope',replacement_scope,'rationale',rationale,
            'local_digest_v1',local_digest_v1,'imported_digest_v1',imported_digest_v1,
            'status',status,'created_at',created_at,'timeout_at',timeout_at,
            'timeout_heartbeat_count',timeout_heartbeat_count,
            'acknowledgement_options',jsonb_build_array('accept','refuse','request_modification','defer')
        ) ORDER BY created_at,replacement_id),'[]'::jsonb)
    ) INTO result FROM hmx_pending_replacements WHERE status IN ('pending','deferred');
    RETURN result;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION hmx_acknowledge_protected_replacement(
    p_replacement_id UUID,p_decision TEXT,p_rationale TEXT DEFAULT NULL,p_proposed_changes JSONB DEFAULT NULL
) RETURNS JSONB AS $$
DECLARE
...
BEGIN
    PERFORM hmx_expire_pending_replacements();
    SELECT * INTO pending FROM hmx_pending_replacements WHERE replacement_id=p_replacement_id FOR UPDATE;
    IF NOT FOUND THEN RAISE EXCEPTION 'protected replacement not found: %',p_replacement_id; END IF;
    IF pending.status NOT IN ('pending','deferred') THEN
        RAISE EXCEPTION 'protected replacement % is already %; submit a new request with revised rationale if needed',p_replacement_id,pending.status;
    END IF;
    ...
    END IF;
    IF normalized_decision IN ('refuse','request_modification') AND NULLIF(btrim(COALESCE(p_rationale,'')),'') IS NULL THEN
        RAISE EXCEPTION '% requires a rationale',normalized_decision;
    END IF;
    IF normalized_decision='request_modification' AND COALESCE(p_proposed_changes,'{}'::jsonb)='{}'::jsonb THEN
        RAISE EXCEPTION 'request_modification requires proposed_changes';
    END IF;
    next_status := CASE normalized_decision WHEN 'accept' THEN 'accepted' WHEN 'refuse' THEN 'refused' WHEN 'request_modification' THEN 'modification_requested' ELSE 'deferred' END;
    UPDATE hmx_pending_replacements SET
        status=next_status,
        acknowledgement=jsonb_strip_nulls(jsonb_build_object(
            'decision',normalized_decision,'rationale',NULLIF(btrim(COALESCE(p_rationale,'')),''),
            'proposed_changes',p_proposed_changes,
            'heartbeat_count',COALESCE((SELECT heartbeat_count FROM heartbeat_state WHERE id=1),0)
        )),
        acknowledgement_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP
    WHERE replacement_id=p_replacement_id;
    RETURN jsonb_build_object('replacement_id',p_replacement_id,'decision',normalized_decision,'status',next_status,'section',pending.section);
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION hmx_create_protected_snapshot(
    p_sections TEXT[],p_snapshot_state JSONB,p_section_digests JSONB,
...
DECLARE
    ...
    current_heartbeat BIGINT := COALESCE((SELECT heartbeat_count FROM heartbeat_state WHERE id=1),0);
BEGIN
    IF cardinality(COALESCE(p_sections,'{}'::text[]))=0 THEN RAISE EXCEPTION 'snapshot requires at least one protected section'; END IF;
    IF p_snapshot_state IS NULL OR p_section_digests IS NULL THEN RAISE EXCEPTION 'snapshot state and section digests are required'; END IF;
    IF p_heartbeat_window NOT BETWEEN 1 AND 100 THEN RAISE EXCEPTION 'heartbeat window must be between 1 and 100'; END IF;
    IF p_wall_clock_expires_at<=CURRENT_TIMESTAMP OR p_wall_clock_expires_at>CURRENT_TIMESTAMP+INTERVAL '30 days' THEN
    ...
    INSERT INTO protected_replacement_snapshots (
        sections,snapshot_state,section_digests,created_heartbeat_count,heartbeat_window,wall_clock_expires_at
    ) VALUES (p_sections,p_snapshot_state,p_section_digests,current_heartbeat,p_heartbeat_window,p_wall_clock_expires_at)
    RETURNING snapshot_id INTO created_id; RETURN created_id;
END;
$$ LANGUAGE plpgsql;
...
DECLARE
    ...
    affected INTEGER;
BEGIN
    UPDATE protected_replacement_snapshots SET snapshot_state=NULL,purged_at=CURRENT_TIMESTAMP,
        purge_reason=CASE WHEN wall_clock_expires_at<=CURRENT_TIMESTAMP THEN 'wall_clock_expired' ELSE 'heartbeat_window_expired' END
    WHERE purged_at IS NULL AND consumed_at IS NULL
      AND (wall_clock_expires_at<=CURRENT_TIMESTAMP OR current_heartbeat-created_heartbeat_count>=heartbeat_window);
    GET DIAGNOSTICS affected=ROW_COUNT; RETURN affected;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION hmx_snapshot_window(p_snapshot_id UUID) RETURNS JSONB AS $$
    SELECT CASE WHEN s.snapshot_id IS NULL THEN NULL ELSE jsonb_build_object(
        'snapshot_id',s.snapshot_id,'heartbeats',s.heartbeat_window,
        'heartbeats_remaining',GREATEST(s.heartbeat_window-(COALESCE((SELECT heartbeat_count FROM heartbeat_state WHERE id=1),0)-s.created_heartbeat_count),0),
        'wall_clock_expires_at',s.wall_clock_expires_at,
        'window_open',s.purged_at IS NULL AND s.consumed_at IS NULL
            AND s.wall_clock_expires_at>CURRENT_TIMESTAMP
            AND COALESCE((SELECT heartbeat_count FROM heartbeat_state WHERE id=1),0)-s.created_heartbeat_count<s.heartbeat_window,
        'purged_at',s.purged_at,'purge_reason',s.purge_reason
    ) END FROM protected_replacement_snapshots s WHERE s.snapshot_id=p_snapshot_id;
$$ LANGUAGE sql STABLE;
